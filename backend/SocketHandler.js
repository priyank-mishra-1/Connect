...
const SocketHandler = (socket) => {
  ...
  socket.on("followUser", async ({ ownId, followingUserId }) => {
    ...
    // if both are following then enable chat
    if (
      ...
      const newChat = new Chats({
        _id:
          user1._id > user2._id ? user1._id + user2._id : user2._id + user1._id,
      });
  ...
  socket.on("fetch-friends", async ({ userId }) => {
    ...
    const friendsList = findCommonElements(
      userData.following,
      userData.followers
    );
  ...
  socket.on(
    "new-message",
    async ({ chatId, id, text, file, senderId, date }) => {
      try {
  ...
  socket.on(
    "create-new-story",
    async ({ userId, username, userPic, fileType, file, text }) => {
